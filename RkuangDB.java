...
public class RkuangDB {
  ...
  public void genMthStatement(String taxid){
    ...
    System.out.print("Initial Stock Balance:");
    initialStockBal(taxid);
    System.out.print("Final Stock Balance:");
    finalStockBal(taxid);
  }

  public void initialStockBal(String taxid) {
    ...
    try (Statement statement = connection.createStatement()){
      ...
      while(rs.next()){
        ...
        q2 = String.format("SELECT stockid, SUM(quantity) FROM Stock_Transactions WHERE taxid='%s' AND stockid='%s' GROUP BY stockid", taxid, stockid);
        try (Statement s2 = connection.createStatement()) {
          ResultSet rs2 = statement.executeQuery(q2);
          while (rs.next()) {
            initQuantity = quantity - rs.getDouble("SUM(quantity)");
          }
        } catch (SQLException e) {
          e.printStackTrace();
        }
